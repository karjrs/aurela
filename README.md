...
- `src/graphql/*.graphql` — hand-written query/mutation documents, e.g. `getUsers.graphql`, `getUser.graphql` (the latter unused for now). Each one's selected fields determine the shape of its generated type — this is a different kind of type than the backend's resolver types (see "Structure" above), not a duplicate of them.
