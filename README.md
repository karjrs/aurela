...
- `src/app/page.tsx` — the home page itself, directly: renders `UsersList`. An earlier landing page (`Hero`/`DayMoments`/`ClosingStatement`) was kept around unused for a while and has since been removed — see git history if it's ever needed again.
- `src/app/page.test.tsx` — Testing Library smoke test for the home page (success/error rendering only), colocated next to `page.tsx` in the same route segment; CRUD behavior (create/edit/delete, validation, not-found responses) is covered separately in `src/components/users/index.test.tsx`, which renders `UsersList` directly and mocks `graphqlClient.request` per operation via `document === XDocument` checks, since one flow can trigger several different requests (e.g. `CreateUser` then a refetching `GetUsers`).
- **Adding a new page**: create a folder under `src/app/` named after the URL segment (e.g. `src/app/settings/` for `/settings`), with a `page.tsx` inside rendering the page's content directly — or delegating to a component in `src/components/` once it's non-trivial, the same way this `page.tsx` delegates to `UsersList`. Colocate `page.test.tsx` (and `loading.tsx`/`error.tsx`/a nested `layout.tsx` if that route needs them) right next to it; nested routes are just nested folders (`src/app/settings/profile/page.tsx` for `/settings/profile`). There used to be a separate `src/views/<name>/page.tsx` indirection with `src/app/page.tsx` re-exporting from it, based on a mistaken assumption that App Router would choke on a colocated `.test.tsx` the way the *legacy* Pages Router does — it doesn't (App Router only treats exact reserved filenames like `page`/`layout`/`loading`/`error` as routes; anything else, including `page.test.tsx`, is safely ignored), so that indirection added nothing and was removed. **Never** name a directory literally `pages` (or `src/pages`) though — that one specific name still auto-activates Next's legacy Pages Router for everything inside it.
