...
- `src/graphql/*.graphql` — hand-written query/mutation documents, e.g. `getUsers.graphql`, `createUser.graphql`. Each one's selected fields determine the shape of its generated type — this is a different kind of type than the backend's resolver types (see "Structure" above), not a duplicate of them.
...
- `src/graphql/types.ts` — **generated, not hand-written.** Per-operation types (`GetUsersQuery`/`GetUsersQueryVariables`, `CreateUserMutation`, ...) plus the runtime `*Document` constants (e.g. `GetUsersDocument`, `CreateUserDocument`) that `graphql-request` sends over the wire — one set per document in `src/graphql/*.graphql`. Both generated files are gitignored, regenerated automatically via `pre*` pnpm script hooks before `dev`/`build`/`typecheck`/`test`.
...
- `src/hooks/useCreateUser.ts` / `useUpdateUser.ts` / `useDeleteUser.ts` — one `useMutation` hook per mutation, same shape: `mutationFn` calls `graphqlClient.request` with the matching `*Document`, `onSuccess` calls `queryClient.invalidateQueries({ queryKey: ["users"] })` to refetch the list. This is the reference pattern for a mutation hook for any future resource — no normalized cache, so invalidation is the only sync mechanism.
...
- `src/lib/utils.ts` — `cn()`, shadcn/ui's helper combining `clsx` (conditional class names) and `tailwind-merge` (resolves conflicting Tailwind utility classes so later ones win). Used by every component in `src/components/common/`.
...
- `src/components/common/` — generic, resource-agnostic UI primitives generated by the `shadcn` CLI (as opposed to `src/components/users/`, which is specific to the users resource). Filenames stay lowercase, matching shadcn's own convention, so future `shadcn add`/updates can still target them. Currently: `button.tsx`, `label.tsx`, `input.tsx`, `field.tsx` (+ its `separator.tsx` dependency), `card.tsx`. Each has a few overrides layered on top of the generated source to keep the pre-migration look: `button.tsx`'s `default`/`secondary`/`destructive` variants and padding, `input.tsx`'s height/padding, `field.tsx`'s `FieldError` text color, and `card.tsx`'s border/radius (see each file for specifics). `components.json`'s `aliases.ui` is pointed at `@/components/common` (not the CLI default `@/components/ui`) so `shadcn add` writes here directly. Note for future `shadcn add` runs: the installed CLI version no longer ships the classic `form.tsx` (`Form`/`FormField`/`FormMessage`) — `shadcn add form` resolves to zero files. Its replacement is `field.tsx`'s `Field`/`FieldLabel`/`FieldError`, which isn't tied to `react-hook-form` the way `form.tsx` was; wire it with plain `register()`/`formState.errors`, as `UserForm.tsx` does.
